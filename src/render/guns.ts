...
/**
 * Гнездо `grip` в ките — верх пистолетной рукояти (часть Grip_* висит от него
 * вниз на ~0,15 и тянется вперёд цевьём). Ладонь обхватывает рукоять ниже
 * верха — на её середине.
 */
const GRIP_RISE = -0.07

const AXIS_Y = new THREE.Vector3(0, 1, 0)

/** Левая ладонь обхватывает цевьё снизу: на столько ниже оси ствола, в единицах кита. */
const HAND_UNDER = 0.06

/**
 * Точка цевья относительно рукояти, в осях оружия (+Z вперёд, +Y вверх), м,
 * при базовом масштабе. Рендер кладёт рукоять в правую ладонь, эту точку —
 * в левую: оружие наклоняется и растягивается, чтобы совпали обе.
 */
export function holdOf(gun: THREE.Object3D): THREE.Vector3 {
  const v: unknown = gun.userData['hold']
  return v instanceof THREE.Vector3 ? v : new THREE.Vector3(0, 0, 0.2)
}
