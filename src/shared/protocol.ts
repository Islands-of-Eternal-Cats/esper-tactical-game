...
export interface UnitView extends MoveView {
  ...
  status: string
  /** То же, что `status`, но знаком: иконка над головой читается без выделения. */
  sign: UnitSign | null
}

/** Состояния, у которых есть иконка. Поза и так видна; знак — для того, чего в позе нет. */
export type UnitSign = 'pinned' | 'seek' | 'reload' | 'far' | 'stuck' | 'yield'
